...
import { SyncLinkInput } from '../../repositories/profile/profile-link.repository';
...
import * as profileLinkService from '../../services/domain/profile-link.service';
...
class ProfileController {
  ...
  getMyLinks = asyncHandler(async (req: Request, res: Response) => {
    const links = await profileLinkService.getLinksForProfile(req.userId!);
  ...
  syncMyLinks = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      const result = await profileLinkService.syncProfileLinks(req.userId!, links);
      return res.success(result, 'Profile links updated successfully');
    } catch (error) {
      if (error instanceof ServiceError && Array.isArray(error.cause)) {
        return res.error(error.message, error.statusCode, error.cause);
      }
      throw error;
    }
  });
